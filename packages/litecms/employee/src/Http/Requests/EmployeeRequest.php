...
class EmployeeRequest extends AbstractRequest
{
    ...
    {
        $this->model = $this->route('employee');

        if (is_null($this->model)) {
            // Determine if the user is authorized to access employee module,
            return $this->user()->can('view', app(config('litecms.employee.employee.model.model')));
        }
